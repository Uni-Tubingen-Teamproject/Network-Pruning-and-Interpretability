...
validation_set = datasets.ImageFolder('/Users/philippholzmann/Desktop/Teamprojekt/imagenette2-320/val',
...
for module_name, module in model.named_modules():
    if hasattr(module, 'weight'):
        for i, amt in enumerate(amounts):
            # Reload the original model before each pruning iteration
            model = torch.hub.load('pytorch/vision:v0.10.0', 'googlenet', weights='GoogLeNet_Weights.DEFAULT')
            model.eval()
            ...
            # Check the percentage of parameters remaining after pruning
            parameters_remaining = float(torch.sum(module.weight != 0)) / float(module.weight.numel())
            print(f"Pruning amount {amt} applied to module {module_name}: {parameters_remaining * 100:.2f}% parameters remaining")
